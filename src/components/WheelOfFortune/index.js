...
class WheelOfFortune extends Component {
  ...
  getRandomIndex = () => {
    return Math.floor(Math.random() * this.numberOfSegments);
  };

  _onPress = () => {
    const duration = this.props.duration || 10000;
    this._angle.setValue(0);
    let newRandomIndex = this.getRandomIndex();

    while (newRandomIndex == this.winner) {
      newRandomIndex = this.getRandomIndex();
    }
    ...
    Animated.timing(this._angle, {
      toValue:
        365 -
        newRandomIndex * (this.oneTurn / this.numberOfSegments) +
        360 * (duration / 1000),
      duration: duration,
      useNativeDriver: true,
    }).start(() => {
      console.log('Animation Over', newRandomIndex);
  ...
  _textRender = (x, y, value, size, i, number) => (
    <Text
      x={x}
      y={y - 50}
                  ...
                  <G key={`arc-${i}`}>
                    ...
                    <G
                      ...
                      {typeof arc.value === 'object'
                        ...
                        : this._textRender(x, y, arc.value, 60, i, number)}
                    </G>
                  </G>
                );
              })}
  ...
  render() {
    console.log('This is new index', this.state.randomIndex);
    return (
      <View style={styles.container}>
        {/** SVG WHEEL  */}
        <TouchableOpacity
